...
/**
 * Local HTTP proxy server that bridges network protocols (SMB/FTP/WebDAV) to video engine.
 * video engine can only play HTTP URLs, so this server streams network files via localhost.
 *
 * Usage:
 * 1. Add a network file stream: addStream(connection, remotePath)
 * 2. Get the local URL: getStreamUrl(streamId)
 * 3. video engine plays: http://127.0.0.1:PORT/stream/<id>
 */
