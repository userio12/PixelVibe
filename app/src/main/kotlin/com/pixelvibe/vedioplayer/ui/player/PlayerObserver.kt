...
class PlayerObserver(
    private val onEvent: (MPVLib.MpvEvent) -> Unit,
    ...
    fun markExiting() { isExiting = true }

    override fun eventProperty(property: String) {
        if (isExiting) return
        mainHandler.post { onPropertyChange(property, null) }
    }

    override fun eventProperty(property: String, value: Boolean) {
        if (isExiting) return
        mainHandler.post { onPropertyChange(property, value) }
    }
    ...
    override fun event(event: MPVLib.MpvEvent) {
        if (isExiting) return
        mainHandler.post { onEvent(event) }
    }

    fun cleanup() { isExiting = true }
}
